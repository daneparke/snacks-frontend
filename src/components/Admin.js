...
const Admin = (props) => {
    let snacks = props.snackList.map(snack => {
        return (
            <div key={`snack ${snack.id}`} className='container snackList'>
                <Link className='removeUnderline' onClick={props.snackIDForReview} id={snack.id} to={`/snack/id/${snack.id}`}>
                ...
                <div className={`adminSnackButtonsCenter ${props.currentUser.admin ? '' : 'hidden'}`}>
                    ...
                    <button className='adminSnackButtons buttonHover' onClick={props.deleteSnack} id={snack.id}>Delete</button>
                </div>
            </div>
        )
    })
    return (
        <>
            ...
            <div>
                <div className='row smallScreen'>{snacks}</div>
            </div>
        </>
    )
}
